...
import { ChangeShiftManagementService } from "./change-shift-management-service";

export class ChangeShiftManagementController {
  static async getAllByCompanyBranchId(
    ...
    try {
      ...
      const result = await ChangeShiftManagementService.getAllByCompanyBranchId(
  ...
  static async getById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await ChangeShiftManagementService.getById(
  ...
  static async validateLetter(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await ChangeShiftManagementService.validateLetter({
  ...
  static async deleteLetter(req: Request, res: Response, next: NextFunction) {
    try {
      const { submission_id, company_branch_id } = req.params;
      const result = await ChangeShiftManagementService.deleteLetter(
        Number(submission_id),
        company_branch_id
      );
      return res.status(200).json({
